...
function renderStats() {
  ...
  els.adminStats.innerHTML = `
    <div class="dash-stat glass"><strong>${products.length}</strong><span>Товарів</span></div>
    <div class="dash-stat glass"><strong>${orders.length}</strong><span>Замовлень</span></div>
    <div class="dash-stat glass"><strong>${formatPrice(revenue)}</strong><span>Сума замовлень</span></div>
    <div class="dash-stat glass"><strong>${lowStock}</strong><span>Мало на складі</span></div>
  `;
}
...
function exportProductsCsv() {
  const rows = [['Назва', 'Бренд', 'Категорія', 'Ціна', 'Склад', 'Статус']];
  getProducts().forEach(p => rows.push([p.title, p.brand, p.category, p.price, p.stock, p.status]));
  exportCsv('smartzone-products.csv', rows);
  showToast('Експорт готовий', 'CSV файл товарів завантажено.');
}
